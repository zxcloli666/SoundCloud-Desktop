...
export function SegmentedTabs<T extends string>({
  ...
  const alignClass = align === 'center' ? 'justify-center mx-auto' : '';

  return (
    <div
      className={`flex items-center gap-1.5 p-1.5 bg-white/[0.02] border border-white/[0.05] rounded-2xl w-fit backdrop-blur-2xl shadow-lg ${alignClass} ${className}`}
    >
      {items.map((tab) => {
