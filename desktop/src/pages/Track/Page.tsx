...
const TrackPageBase = () => {
  ...
  const playbackQueue = track ? [track, ...relatedTracks] : relatedTracks;
  const { isCurrentPlaying, togglePlay } = useTrackPlayback(track, playbackQueue);
    ...
    <div className="p-6 pb-4 space-y-7 animate-fade-in-up">
        ...
        <div className="relative flex items-center gap-7 p-7">
          ...
          <div className="flex-1 min-w-0 py-2">
            ...
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              />
              <TrackRepostButton trackUrn={track.urn} count={track.reposts_count} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="flex items-center gap-5 px-1 flex-wrap">
        <div className="flex items-center gap-1.5 text-[12px] text-white/30">
          ...
          <span className="tabular-nums font-medium">{toCompactCount(track.playback_count)}</span>
        ...
        <div className="flex items-center gap-1.5 text-[12px] text-white/30">
          ...
          <span className="tabular-nums font-medium">{toCompactCount(track.reposts_count)}</span>
          <span className="text-white/15">{t('track.reposts')}</span>
        </div>
        <div className="flex items-center gap-1.5 text-[12px] text-white/30">
          <MessageCircle size={13} className="text-white/20" />
          <span className="tabular-nums font-medium">{toCompactCount(track.comment_count)}</span>
        ...
        <div className="flex items-center gap-1.5 text-[12px] text-white/25 ml-auto">
          <Clock size={12} />
          <span className="tabular-nums">{toHourMinSec(track.duration)}</span>
        </div>
      </section>
            ...
            {relatedLoading ? (
              ...
              <div className="space-y-1">
                {relatedTracks.map((rt) => (
                  <TrackRelatedRow key={rt.urn} track={rt} queue={relatedTracks} />
                ))}
              </div>
            )}
          </section>
