...
export function useLikedTracks(limit = 10) {
    return useQuery({
        ...
        queryFn: () =>
            api<TrackListResponse>(`/me/likes/tracks?limit=${limit}`),
    })
}
...
export function useUserWebProfiles(userUrn: string | undefined) {
    return useQuery({
        ...
        queryFn: () =>
            api<WebProfile[]>(
                `/users/${encodeURIComponent(userUrn!)}/web-profiles`
            ),
        enabled: !!userUrn,
        refetchOnMount: 'always',
    })
...
export function useMyLikedPlaylists(limit = 100) {
    return useQuery({
        ...
        queryFn: () =>
            api<PlaylistListResponse>(`/me/likes/playlists?limit=${limit}`),
    })
}
...
export function useSearchPlaylists(q: string) {
    const query = useInfiniteQuery({
        ...
        initialPageParam: undefined as PageParam | undefined,
        getNextPageParam: (last, _all, _lastPageParam) =>
            extractPagination(last.next_href),
        enabled: !!q.trim(),
    })
