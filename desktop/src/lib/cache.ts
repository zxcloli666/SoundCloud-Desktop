...
export async function getCachedData(urn: string): Promise<ArrayBuffer | null> {
    ...
    try {
        ...
        if (info.size < MIN_MP3_SIZE) {
            console.warn(
                `💾 [Cache] MISS (file too small - ${info.size} bytes) for: ${urn}. Deleting.`
            )
            await remove(path).catch(() => {})
            return null
        }
    ...
    const promise = (async () => {
        try {
            ...
            if (isValidMp3(buffer)) {
                console.log(
                    `💾 [Cache] Download complete for ${urn}. Valid MP3. Saving...`
                )
                const path = await filePath(urn)
                await writeFile(path, new Uint8Array(buffer)).catch((e) =>
