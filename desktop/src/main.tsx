import { QueryClientProvider } from '@tanstack/react-query';
import { invoke } from '@tauri-apps/api/core';
import React from 'react';
import ReactDOM from 'react-dom/client';
import App from './App';
import { initDiscordIntegration } from './integrations/discord.ts';
import { initTrayIntegration } from './integrations/tray.ts';
import { setCacheServerPort } from './lib/cacheServerPort.ts';
import { queryClient } from './lib/queryClient.ts';
import './i18n';
import './index.css';

if (import.meta.env.DEV) {
  const script = document.createElement('script');
  script.src = 'https://unpkg.com/react-scan/dist/auto.global.js';
  script.crossOrigin = 'anonymous';
  document.head.appendChild(script);
}

initDiscordIntegration();
initTrayIntegration();

invoke<number>('get_cache_server_port')
  .then((port) => {
    setCacheServerPort(port);
    console.log(`[CacheServer] Port received: ${port}`);
  })
  .catch((e) => console.warn('[CacheServer] Failed to get port:', e));

ReactDOM.createRoot(document.getElementById('root') as HTMLElement).render(
  <React.StrictMode>
    <QueryClientProvider client={queryClient}>
      <App />
    </QueryClientProvider>
  </React.StrictMode>,
);
